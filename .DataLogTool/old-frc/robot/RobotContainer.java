...
public class RobotContainer {
	...
	private final HoldElevator m_elevatorCommand = new HoldElevator(m_elevatorSubsytem);
	...
	private void configureBindings() {
		...
		new JoystickButton(Constants.emergencyController, XboxController.Button.kA.value).whileTrue(m_driveSubsystem.runEnd(
			() -> {
				...
				m_driveSubsystem.driveRobotSpeed(new ChassisSpeeds(Units.MetersPerSecond.of(speed * Math.cos(thetaRads)), Units.MetersPerSecond.of(speed * Math.sin(thetaRads)), Units.DegreesPerSecond.of(thetaPerSecond)));
			}, () -> {
				m_driveSubsystem.setChassisVoltage(0);
			}
		));
