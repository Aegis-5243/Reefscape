...
public class UtilFunctions {
    /**
     * This function takes a joystick input from -1 to 1 and removes the center of the stick. This is
     * because Xbox joysticks have awful centering
     *
     * @param input Joystick input to manipulate
     * @param deadband How much of the center we need to remove (Xbox 360 controllers was around 0.2,
     *     Xbox One 0.13)
    ...
    /**
     * This function takes a input in degrees and makes it -180 to 180*. If you are in radians, use
     * MathUtil.angleModulus() from WpiLib
     *
    ...
    /**
     * This uses the Preferences API to save settings over power cycles. This is different in that you
     * don't have to set the default value, it will set it for you.
     *
    ...
    /**
     * This creates a NT subscriber so we don't have to keep querying the key in the table to get the
     * value. It will locate the key in the Preferences table still.
     *
     * @param key The parameter you want to get (slashes are allowed)
     * @param backup The value to use if the key is missing
    ...
    public static DoubleSubscriber getSettingSub(String key, double backup) {
      DoubleTopic topic =
          NetworkTableInstance.getDefault().getDoubleTopic(checkKey("/Preferences/" + key));
    ...
    public static BooleanSubscriber getSettingSub(String key, boolean backup) {
      BooleanTopic topic =
          NetworkTableInstance.getDefault().getBooleanTopic(checkKey("/Preferences/" + key));
     ...
     * @param listener Function to run when the value changes. Could be a function taking a
     *     NetworkTableEvent parameter or a lambda function.
    ...
    public static int onNtChange(String key, Consumer<NetworkTableEvent> listener) {
      ...
      return NetworkTableInstance.getDefault()
          .addListener(sub, EnumSet.of(NetworkTableEvent.Kind.kValueAll), listener);
    }
    ...
    /**
     * This function adds a periodic function to the schedule. This will run after the main loop
     * finishes.
     *
